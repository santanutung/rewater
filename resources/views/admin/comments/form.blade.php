...
<div class="block full">
    <div class="block-title">
        ...
        <div class="block-options pull-right">
            <a href="{{ route('admin.comments.index') }}" class="btn  btn-danger ">Back</a>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12">
            <form id="form-validation" class="form-horizontal form-bordered" method="post"
                ...
                @endif
                <fieldset>
                    <div class="form-group  @error('art') has-error @enderror">
                        <label class="col-md-3 control-label" for="example-select">Art  <span
                                    class="text-danger">*</span></label>
                        <div class="col-md-5">
                            <select id="example-select2" name="art" class="form-control select-select2" size="1" data-placeholder="Choose one..">
                                 <option></option>

                                @foreach ($arts as $art)
                                    <option @if(isset($comment)){{($comment->art_id==$art->id)? 'selected':''}}  @endif   value="{{ $art->id }}">{{ $art->name }}</option>
                        ...
                        <div class="col-md-5">
                            <select id="example-select" name="rating" class="form-control" size="1">
                                <option disabled>Please select</option>
                                 <option @if(isset($comment)){{($comment->rating==0)? 'selected':''}}  @endif value="0">0</option>
                                <option @if(isset($comment)){{($comment->rating==1)? 'selected':''}}  @endif value="1">1</option>
                                <option @if(isset($comment)){{($comment->rating==2)? 'selected':''}}  @endif value="2">2</option>
                                <option @if(isset($comment)){{($comment->rating==3)? 'selected':''}}  @endif value="3">3</option>
                                <option @if(isset($comment)){{($comment->rating==4)? 'selected':''}}  @endif value="4">4</option>
                                <option @if(isset($comment)){{($comment->rating==5)? 'selected':''}}  @endif value="5">5</option>
                            </select>
                            @error('rating')
                                <span class="help-block">{{ $message }}</span>
                            @enderror
